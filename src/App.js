...
const App = ({ signOut, user }) => {
  ...
  // Add this useEffect to get user information
  useEffect(() => {
    const getUserInfo = async () => {
      try {
        const currentUser = await getCurrentUser();
        const { username, userId, signInDetails } = currentUser;
        setUserEmail(signInDetails.loginId || username); // Use loginId (email) or fallback to username
      } catch (error) {
      ...
      <Box sx={{ 
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
          <Typography variant="body2">
            {userEmail} {/* Changed from user.attributes.email */}
          </Typography>
          <Button 
            color="inherit" 
            onClick={async () => {
